										...
										uint32_t num_entries)
{
	...
	for (i = 0; i < num_entries; i++) {
		...
		if ((uintptr_t)data & (0x3LU)) {
			aligned_data = (uint8_t *)tegrabl_memalign(4, size);
...
#else
		status = write_a_b_partition(entry->partname, data,
									 entry->image_size);
#endif
													...
													const char *suffix)
{
	...
	if (btype != BLOB_UPDATE) {
		pr_error("%s Payload is not of UPDATE type\n", __func__);
		goto end;
	}
	...
	if (!num_entries) {
		...
		pr_error("%s No entries in payload\n", __func__);
		goto end;
	}
