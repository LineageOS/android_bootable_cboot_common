 ...
 */
tegrabl_error_t (*publish_partition[])(tegrabl_bdev_t *bdev,
	...
	list_for_every_entry(storage_list, entry,
		...
		for (i = 0; i < num_partitions; i++) {
			...
			if (tegrabl_a_b_match_part_name(partition_info[i].name,
											partition_name)) {
#else
			if (!strcmp(partition_info[i].name, partition_name)) {
#endif
				break;
			}
		}
		...
		bool secure)
{
	...
	if (!partition_info || !partition->block_device) {
	...
	error = tegrabl_blockdev_erase(partition->block_device,
			partition_info->start_sector, partition_info->num_sectors, secure);
		...
		uint32_t *start)
{
	...
	*start = partition_info->start_sector;
				...
				int64_t offset, enum tegrabl_partition_seek origin)
{
	...
	case TEGRABL_PARTITION_SEEK_END:
		new_offset = partition_info->total_size;
		break;
	default:
...
{
	if (!partition || !partition->partition_info || !partition->block_device) {
		return -1;
	}
...
{
	tegrabl_aio_t *aio = NULL;
	struct tegrabl_partition_info *partition_info = NULL;

	if (!partition || !buf || !num_sectors) {
		goto fail;
	}
	...
	aio = tegrabl_blockdev_async_read_block(partition->block_device,
			buf, partition_info->start_sector + start_sector, num_sectors);
		...
		uint64_t num_sectors)
{
	...
	aio = tegrabl_blockdev_async_write_block(partition->block_device,
			buf, partition_info->start_sector + start_sector, num_sectors);
...
tegrabl_error_t tegrabl_partition_publish(tegrabl_bdev_t *dev, off_t offset)
{
	...
	if (dev->published) {
		...
		for (i = 0; i < (int32_t)ARRAY_SIZE(publish_partition); i++) {
			if (publish_partition[i]) {
				error = publish_partition[i](dev, offset, &partitions, &num);
				if (TEGRABL_NO_ERROR != error) {
					continue;
				}
				pr_info("Found %d partitions in %08x device\n",
						num, dev->device_id);
...
tegrabl_error_t tegrabl_partition_manager_init(void)
{
	...
	pr_debug("Initializing partition manager\n");
...
static tegrabl_error_t tegrabl_partition_add_verify_node(char *partition_name)
{
	...
	list_for_every_entry(&verify_list, verify_handle,
		...
		if (strcmp(verify_handle->name, partition_name) == 0) {
			error = TEGRABL_ERROR(TEGRABL_ERR_ALREADY_EXISTS, 0);
			goto fail;
		}
	}
