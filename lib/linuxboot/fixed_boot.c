...
tegrabl_load_from_partition(struct tegrabl_kernel_bin *kernel,
			    ...
			    void **kernel_dtbo, void **vendor_boot_img_load_addr,
			    void *data, uint32_t data_size,
			    bool boot_to_recovery)
{
	uint32_t boot_img_size, vendor_boot_img_size;
	...
	if (tegrabl_get_partition_name(TEGRABL_BINARY_VENDOR_KERNEL, 0, partition_name) == TEGRABL_NO_ERROR &&
		...
		err = tegrabl_load_binary(TEGRABL_BINARY_VENDOR_KERNEL, vendor_boot_img_load_addr,
					&vendor_boot_img_size);
		if (err != TEGRABL_NO_ERROR) {
			*vendor_boot_img_load_addr = NULL;
			goto boot_image_load_done;
		}
		vendor_boot_img_size = BOOT_IMAGE_MAX_SIZE;
		err = tegrabl_verify_vendor_boot_img_hdr(*vendor_boot_img_load_addr, vendor_boot_img_size);
		if (err != TEGRABL_NO_ERROR) {
			*vendor_boot_img_load_addr = NULL;
			goto boot_image_load_done;
		}
