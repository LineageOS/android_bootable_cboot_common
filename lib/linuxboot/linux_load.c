...
static tegrabl_bootimg_header *android_hdr;
#endif

#define HAS_BOOT_IMG_HDR(ptr)	\
			((memcmp((ptr)->magic, BOOT_MAGIC, BOOT_MAGIC_SIZE) == 0) ? true : false)

#if defined(CONFIG_ENABLE_L4T_RECOVERY)
struct tegrabl_kernel_bootctrl dummy_kernel_bootctrl = {
...
static tegrabl_error_t extract_ramdisk(void *boot_img_load_addr, void* vendor_boot_img_load_addr)
{
	tegrabl_bootimg_header *hdr = NULL;
	...
	struct vendor_boot_img_hdr_v3 *vhdr = (struct vendor_boot_img_hdr_v3*)vendor_boot_img_load_addr;
											...
											uint32_t data_size)
{
	...
	void *boot_img_load_addr = NULL;
	void *ramdisk_load_addr = NULL;
	void *vendor_boot_img_load_addr = NULL;
	uint32_t kernel_size = 0;
	...
	if (HAS_BOOT_IMG_HDR((tegrabl_bootimg_header *)boot_img_load_addr)) {
		err = extract_ramdisk(boot_img_load_addr, vendor_boot_img_load_addr);
		if (err != TEGRABL_NO_ERROR) {
