...
static enum nct_id mac_type_nct_id[MAC_ADDR_TYPE_MAX] = {
				NCT_ID_WIFI_ADDR,
				NCT_ID_BT_ADDR,
				NCT_ID_ETH_ADDR,
};
...
static tegrabl_error_t nct_get_mac_addr(void *param)
{
	...
	char *buf = (char *)mac_addr_info->buf;
	...
	create_mac_addr_string(buf, data, true);
...
fail:
	pr_info("%s: Number of valid board id found from NCT: %d\n", __func__,
			id_info->count);
