...
#define UFS_RPMB_DEBUG			0
#define UFS_RPMB_DUMP_DATA(x...)			\
	do {						\
		if (UFS_RPMB_DEBUG) {			\
			ufs_rpmb_dump_data(x); }	\
	} while (0)
 ...
 *  @return void
 */
static void ufs_rpmb_dump_data(const char *func, const char *str,
 ...
 */
static inline rpmb_u16_t ufs_convert_to_rpmb_u16(uint16_t u16_val)
{
	rpmb_u16_t rpmb_val;
 ...
 */
static inline rpmb_u32_t ufs_convert_to_rpmb_u32(uint32_t u32_val)
{
	rpmb_u32_t rpmb_val;
 ...
 */
static inline uint16_t ufs_convert_from_rpmb_u16(rpmb_u16_t rpmb_u16)
{
	uint16_t val;
 ...
 */
static inline uint32_t ufs_convert_from_rpmb_u32(rpmb_u32_t rpmb_u32)
{
	uint32_t val;
 ...
 */
static tegrabl_error_t ufs_compare_mac(ufs_rpmb_frame_t *frame,
										 uint8_t *mac_buf)
{
 ...
 *  @return TEGRABL_ERR_INVALID if there is a mismatch TEGRABL_NO_ERROR otherwise.
 */
static tegrabl_error_t ufs_compare_nonce(ufs_rpmb_frame_t *frame, uint8_t *nonce)
{
 ...
 *  @return TEGRABL_ERR_INVALID if there is a mismatch TEGRABL_NO_ERROR otherwise.
 */
static tegrabl_error_t ufs_rpmb_check_response(ufs_rpmb_frame_t *frame,
					uint16_t resp)
{
	uint16_t val16;
	...
	/* Check result. */
	val16 = ufs_convert_from_rpmb_u16(frame->result);
	...
	/* Check response type. */
	val16 = ufs_convert_from_rpmb_u16(frame->req_or_resp);
	pr_trace("val16 is %d\n", val16);
	if (val16 != resp) {
		error = TEGRABL_ERROR(TEGRABL_ERR_INVALID, 0);
		goto fail;
...
static tegrabl_error_t ufs_rpmb_generate_sha256_hash(uintptr_t input,
...
static tegrabl_error_t ufs_rpmb_calculate_mac(ufs_rpmb_key_t *key,
					ufs_rpmb_frame_t *frame,
					uint8_t *out_mac)
{
	 ...
	 * 284 bytes starting at ufs_rpmb_frame_t.data.
	 */
	...
	*/
	...
	error = ufs_rpmb_generate_sha256_hash((uintptr_t)buf,
...
fail:
	...
	if (error != TEGRABL_NO_ERROR) {
		pr_error("UFS RPMB calculate MAC: exit error = %08X\n", error);
	}
...
{
	ufs_rpmb_frame_t *req_frame, *resp_frame;
	...
	req_frame->req_or_resp =
		ufs_convert_to_rpmb_u16(RPMB_REQ_GET_COUNTER);
	memcpy(req_frame->nonce, nonce, sizeof(req_frame->nonce));

	ufs_rpmb_dump_data(__func__, "Dumping pre-request frame:",
			(uint8_t *)req_frame, sizeof(*req_frame));
	ufs_rpmb_dump_data(__func__, "Dumping pre-response frame:",
			(uint8_t *)resp_frame, sizeof(*resp_frame));

	/* Send read request here. */
	error = ufs_rpmb_io(0, rpmb_context, context);
	if (error != TEGRABL_NO_ERROR) {
		goto fail;
	}

	ufs_rpmb_dump_data(__func__, "Dumping post-response frame:",
			(uint8_t *)resp_frame, sizeof(*resp_frame));

	/* Check response frame. */
	error = ufs_rpmb_check_response(resp_frame, RPMB_RESP_GET_COUNTER);
	if (error != TEGRABL_NO_ERROR) {
		goto fail;
	}

	/* Generate MAC. */
	error = ufs_rpmb_calculate_mac(key, resp_frame, mac_buf);
	if (error != TEGRABL_NO_ERROR) {
		goto fail;
	}

	/* Compare MACs. */
	error = ufs_compare_mac(resp_frame, mac_buf);
	if (error != TEGRABL_NO_ERROR) {
		goto fail;
	}

	*counter = ufs_convert_from_rpmb_u32(resp_frame->write_counter);
	pr_trace("counter value is %0x\n", *counter);
fail:
	if (error != TEGRABL_NO_ERROR) {
		pr_error("UFS RPMB get write counter: exit error = %08X\n", error);
	}
	return error;
}
 ...
 *  @return TEGRABL_NO_ERROR on success and failure condition otherwise.
 */
static tegrabl_error_t ufs_rpmb_read(tegrabl_bdev_t *dev, ufs_rpmb_key_t *key,
				...
				struct tegrabl_ufs_context *context)
{
	...
	/* Init request frame. */
	req_frame->address = ufs_convert_to_rpmb_u16(addr);
	req_frame->block_count = ufs_convert_to_rpmb_u16(1);
	req_frame->req_or_resp = ufs_convert_to_rpmb_u16(RPMB_REQ_READ);
	memcpy(req_frame->nonce, nonce, sizeof(req_frame->nonce));
	...
	/* Check response frame. */
	error = ufs_rpmb_check_response(resp_frame, RPMB_RESP_READ);
	if (error != TEGRABL_NO_ERROR) {
		goto fail;
	}

	/* Compare nonce. */
	error = ufs_compare_nonce(resp_frame, nonce);
	if (error != TEGRABL_NO_ERROR) {
		goto fail;
	}
...
fail:
	if (error != TEGRABL_NO_ERROR) {
		pr_error("UFS RPMB read: exit error = %08X\n", error);
	}
	return error;
}
 ...
 *  @return TEGRABL_NO_ERROR on success and failure condition otherwise.
 */
static tegrabl_error_t ufs_rpmb_write(tegrabl_bdev_t *dev, ufs_rpmb_key_t *key,
				uint16_t addr, uint8_t *bufp, uint32_t buflen,
				ufs_rpmb_context_t *rpmb_context,
				struct tegrabl_ufs_context *context)
{
	ufs_rpmb_frame_t *req_frame, *req_resp_frame, *resp_frame;
	uint32_t counter = 0;
	tegrabl_error_t error = TEGRABL_NO_ERROR;
	...
	error = ufs_rpmb_get_write_counter(dev, key, &counter,
	...
	/* Init request frame. */
	req_frame->write_counter = ufs_convert_to_rpmb_u32(counter);
	req_frame->address = ufs_convert_to_rpmb_u16(addr);
	req_frame->block_count = ufs_convert_to_rpmb_u16(1);
	req_frame->req_or_resp = ufs_convert_to_rpmb_u16(RPMB_REQ_WRITE);
	memcpy(req_frame->data, bufp, buflen);

	/* Generate MAC. */
	error = ufs_rpmb_calculate_mac(key, req_frame, req_frame->key_or_mac);
	if (error != TEGRABL_NO_ERROR) {
		goto fail;
	}
	...
	req_resp_frame->req_or_resp =
		ufs_convert_to_rpmb_u16(RPMB_REQ_GET_RESULT);

	ufs_rpmb_dump_data(__func__, "Dumping pre-request frame:",
			(uint8_t *)req_frame, sizeof(*req_frame));
	ufs_rpmb_dump_data(__func__, "Dumping pre-req-response frame:",
			(uint8_t *)req_resp_frame, sizeof(*req_resp_frame));
	ufs_rpmb_dump_data(__func__, "Dumping pre-response frame:",
			(uint8_t *)resp_frame, sizeof(*resp_frame));

	/* Send request to RPMB partition. */
	error = ufs_rpmb_io(1, rpmb_context, context);
	if (error != TEGRABL_NO_ERROR) {
		goto fail;
	}
	...
	/* Check response frame. */
	error = ufs_rpmb_check_response(resp_frame, RPMB_RESP_WRITE);
	if (error != TEGRABL_NO_ERROR) {
		goto fail;
	}
...
fail:
	if (error != TEGRABL_NO_ERROR) {
		pr_error("UFS RPMB write: exit error = %08X\n", error);
	}
	return error;
}
 ...
 *  @return TEGRABL_NO_ERROR on success and failure condition otherwise.
 */
static tegrabl_error_t ufs_rpmb_test(tegrabl_bdev_t *dev,
	ufs_rpmb_key_t *key, ufs_rpmb_context_t *rpmb_context,
	struct tegrabl_ufs_context *context)
{
	...
	error = ufs_rpmb_write(dev, key, 0, write_buf, RPMB_DATA_SIZE,
				rpmb_context, context);
	if (error != TEGRABL_NO_ERROR) {
		goto fail;
	}
	...
	error = ufs_rpmb_read(dev, key, 0, read_buf, RPMB_DATA_SIZE,
				rpmb_context, context);
	if (error != TEGRABL_NO_ERROR) {
		goto fail;
	}
...
{
	ufs_rpmb_key_t key;
	ufs_rpmb_context_t *rpmb_context = NULL;
	ufs_rpmb_frame_t *req_frame, *req_resp_frame, *resp_frame;
	tegrabl_error_t error = TEGRABL_NO_ERROR;
	...
	ufs_rpmb_dump_data(__func__, "Dumping key data",
	...
	/* Init request frame. */
	req_frame->req_or_resp =
		ufs_convert_to_rpmb_u16(RPMB_REQ_PROGRAM_KEY);
	...
	/* Check response frame. */
	error = ufs_rpmb_check_response(resp_frame, RPMB_RESP_PROGRAM_KEY);
	if (error != TEGRABL_NO_ERROR) {
		goto fail;
	}
	error = ufs_rpmb_test(bdev, &key, rpmb_context, context);
	if (error != TEGRABL_NO_ERROR) {
		goto fail;
	}
